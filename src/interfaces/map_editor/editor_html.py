...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Paint 방향 토글 */
    .dir-toggle { display:flex; gap:6px; margin-top:8px; }
    .dir-toggle button {
    ...
    /* 우클릭 컨텍스트 메뉴 (Phase 3 에서 사용) */
    .ctx-menu {
      position:fixed; background:var(--surface);
      border:1px solid var(--border-strong); border-radius:var(--radius-sm);
      box-shadow:0 4px 14px rgba(15,23,42,0.15);
      padding:4px; min-width:140px; z-index:1000;
      display:none;
    }
    ...
    .ctx-menu button {
      ...
      text-align:left; font-size:12px; color:var(--ink);
      border-radius:4px;
    }
...
<body>
  <div class="layout">
    ...
    <aside class="side">
      ...
      <div class="panel">
        ...
        <!-- Paint 모드 옵션 -->
        <div id="paint-options" style="margin-top:12px;">
          <div class="hint" style="font-size:11.5px; color:var(--muted);">
            드래그로 화살표 쫙 → 가까운 엣지 일괄 방향
          </div>
          <div class="dir-toggle">
            <button data-dir="uni" class="active">→ 단방향<span class="hint-key">drag</span></button>
            <button data-dir="bi">↔ 양방향<span class="hint-key">Shift</span></button>
          </div>
        </div>
      </div>
  ...
  <!-- 우클릭 컨텍스트 메뉴 (노드 우클릭 시) -->
  <div class="ctx-menu" id="node-ctx-menu">
    <button data-stamp="station">🟢 Station 으로</button>
    <button data-stamp="charger">🔵 Charger 로</button>
    <button data-stamp="holding">⚪ Holding 으로</button>
    <button data-stamp="siding">🟡 Siding 으로</button>
    <button data-stamp="reset">⬜ Standard 로 (Reset)</button>
  </div>
  ...
  <script>
    ...
    let paintDir = "uni";            // uni / bi (Paint 모드 방향 토글)
    ...
    document.querySelectorAll(".dir-toggle button").forEach(btn => {
      btn.addEventListener("click", () => {
        paintDir = btn.dataset.dir;
        document.querySelectorAll(".dir-toggle button").forEach(b => b.classList.toggle("active", b === btn));
      });
    });
    ...
    document.getElementById("map").addEventListener("pointerup", (e) => {
      ...
      if (paintTrajectory) {
        if (paintTrajectory.length >= 2) {
          applyPaint(paintTrajectory, e.shiftKey, e.altKey);
        }
        paintTrajectory = null;
        render();
      }
    ...
    document.getElementById("map").addEventListener("pointerleave", (e) => {
      // 맵 밖으로 나가면 진행 중인 인터랙션 cancel
      document.querySelector(".map-shell").classList.remove("dragging");
      if (paintTrajectory && paintTrajectory.length >= 2) {
        applyPaint(paintTrajectory, false, false);
      }
      paintTrajectory = null;
      isPanning = false; panStart = null;
      render();
    ...
    function applyPaint(trajectory, shiftKey, altKey) {
      // 키 조합 임시 override:
      //   기본 → paintDir (uni/bi)
      //   Shift → bi (양방향 강제)
      //   Alt   → uni (역방향 강제, trajectory 와 반대로)
      const effectiveDir = shiftKey ? "bi" : paintDir;
    ...
    document.getElementById("map").addEventListener("contextmenu", (e) => {
      ...
      if (!nodeHit) return;
      e.preventDefault();
      ctxTargetNodeId = nodeHit.dataset.nodeId;
    ...
    document.addEventListener("click", (e) => {
      if (!e.target.closest("#node-ctx-menu")) {
        document.getElementById("node-ctx-menu").classList.remove("show");
      }
    });
    document.getElementById("node-ctx-menu").addEventListener("click", (e) => {
      const btn = e.target.closest("button[data-stamp]");
      if (!btn || !ctxTargetNodeId) return;
      const n = nodeById.get(ctxTargetNodeId);
      applyStamp(n, btn.dataset.stamp);
      render();
      showToast(`${n.id} → ${btn.dataset.stamp}`);
      document.getElementById("node-ctx-menu").classList.remove("show");
    });
