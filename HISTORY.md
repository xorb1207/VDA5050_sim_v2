...
## 2026-05-24
- PM Agent System smoke test 완료

## 2026-05-25 scope test

---

## 2026-05-25 — F1a Multi-graph / 이기종 Fleet 완료

### 구현 범위

**엔진 레이어**
- `Edge.graph_idx` (int, default 0): 엣지 별 fleet 레인 인덱스
- `Fleet` 도메인 클래스: id, graph_idx, capabilities, color, count, max_speed_mps, priority
- `MapGraph.get_path(fleet=)`: fleet 이 있으면 fleet.graph_idx 와 일치하는 엣지만 탐색
- `Dispatcher.get_eligible_agvs(required_capability)`: capability 매칭 fleet 의 AGV만 반환
- `AGV.fleet` 필드 + 7개 `get_path` 호출에 `fleet=self.fleet` 연결

**Quickrun UI**
- AGV 마커: fleet 색 ring (다중 fleet 시) + 상태 색 fill (기존 유지)
- Fleet KPI 카드: 다중 fleet 시 `FLEET KPI` 섹션에 fleet별 task/h, utilization, head-on 표시
- Fleet count 슬라이더: 다중 fleet 시 fleet별 개별 슬라이더, 단일 fleet 시 기존 슬라이더 유지
- 임포트 맵 드롭다운: 업로드된 맵을 topology dropdown에 자동 추가
- Fleet legend: SVG 맵 내 fleet 색 범례

**임포터 강화**
- `_import_nodes()`: `type/is_charger/is_station/is_holding_point/capability` 명시 필드 인식
- 기존 추론 파이프라인 유지 (backward compatible)

**검증 맵**
- `scripts/generate_synthetic_3fleet.py`: TYPE_A (overhead/graph_idx=0), TYPE_B (floor/1), TYPE_C (scan/2) 3-fleet 합성 맵 생성기
- 노드 ID: `{A|B|C}_{C|W1-5|HP}`, `ST_{OVERHEAD|FLOOR|SCAN}_{01-04}`
- `generate_3fleet_map()` 함수 직접 호출 가능 (disk I/O 없이 dict 반환)

### 테스트 결과
- T66 unit tests: **20/20 PASS** (graph isolation, capability dispatch, fleet KPI attribution)
- 통합 테스트: **63/63 PASS** (기존 단일 fleet 동작 regression 없음)
- Backend API 검증: `/init` fleets 3개, WS tick `by_fleet` {TYPE_A, TYPE_B, TYPE_C}, `agv.fleet_id` 정상 포함 확인
- 단일 fleet (topology A): fleet KPI 카드/fleet 슬라이더 숨김 → 기존 UX 변화 없음

---

## 2026-05-31 — GAP-D RMF YAML import/export 완료

### 목표
폐쇄망 ICS 시스템의 YAML 포맷 (Open-RMF building_map) 직접 import/export.

### 발견 사항
**모든 핵심 기능이 이미 완전히 구현되어 있었음!**

기존 코드:
- `import_map_yaml()` — RMF YAML → ImportedMap 변환 (graph_idx, fleets/demands 섹션 포함)
- `import_map()` — 확장자 자동 감지 (.yaml/.yml/.json)
- Quickrun `/upload-map` — `map_yaml` 필드로 YAML 문자열 받음
- `export_to_rmf_yaml()` — MapGraph → RMF building.yaml dict 반환 (typed params)

### 버그 수정
1. **Typed params 파싱 누락**
   - 문제: `export_to_rmf_yaml()`은 typed params (`[type_code, value]`)로 export하지만, `_import_from_yaml_level()`은 일반 값만 파싱
   - 해결: `_parse_rmf_param()` 헬퍼 추가 — typed params와 일반 값 모두 파싱

2. **5-element vertex 포맷 파싱 누락**
   - 문제: export는 `[x, y, elevation, name, params]` (5 elements)이지만, import는 `[x, y, name, params]` (4 elements)만 기대
   - 해결: `_import_from_yaml_level()` 수정 — 5-element 포맷도 파싱

### 작업 내용
- `tests/integration/test_yaml_import_export.py` 작성 (T69-1~T69-8)
  - 기본 vertices/lanes 파싱
  - lanes의 graph_idx 인식 (F1a 호환)
  - fleets 섹션 파싱
  - 확장자 자동 감지 (.yaml/.yml/.json)
  - /upload-map YAML 처리 로직
  - 비표준 YAML 명확한 에러
  - roundtrip (JSON → YAML export → YAML import → 동등)
- `scripts/export_to_yaml.py` CLI 유틸리티 추가
- CLAUDE.md 우선순위 업데이트 (GAP-D 완료 → Background image overlay로 전환)

### 검증 결과
- 신규 테스트: **8/8 PASS**
- 통합 테스트: **70/70 PASS** (회귀 없음)
- 실제 RMF YAML: `fab_nav_graph.yaml` import 성공 (82 nodes, 89 edges)
- Spec Final Verification 예제: `/tmp/test.yaml` → preview HTML 정상 생성
- YAML export: RMF 표준 typed params 형식으로 정상 export
- Roundtrip: JSON → MapGraph → YAML → ImportedMap → 동일 결과 확인

### Acceptance Criteria
- [x] 8개 테스트 PASS
- [x] 기존 JSON 시나리오 무영향
- [x] Open-RMF traffic-editor YAML 직접 import 가능
- [x] 확장자 자동 감지 작동
- [x] Typed params import/export 동작
- [x] Roundtrip 성공

### 소요 시간
~0.3일 (견적 ~0.7일 대비 -57%, 대부분 기능이 이미 구현되어 있었음)

### 다음 우선순위
**Background image overlay** — 실 도면 PNG 위에 그래프 그리기 (견적 ~1일)
